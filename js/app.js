    ...
    const subjectsHtml = subjectStats.map(stat => {
        ...
        return `
            <div class="exam-subject-card" data-subject="${stat.subject}">
                ...
                <!-- Data Attribute for Container -->
                <div class="exam-list-container" style="display:flex; flex-direction:column; gap:8px; margin-top:10px; max-height:400px; overflow-y:auto; padding-right:5px;">
                    ${listHtml || '<div style="font-size:0.8rem; opacity:0.5; text-align:center; padding:10px;">Nothing due! 🎉</div>'}
                </div>
            </div>
        `;
    }).join('');
    ...
    if (examView) {
        examView.addEventListener('click', (e) => {
            ...
            if (card && card.dataset.url) {
                loadPaper(card.dataset.url);
            }
        });
    }
    // Also for main list view
    const mainList = document.getElementById('paper-list');
    if (mainList) {
        mainList.addEventListener('click', (e) => {
            // We need to match the structure in renderList (which I need to update similarly)
            const item = e.target.closest('[data-paper-url]');
            if (item && item.dataset.paperUrl) {
                loadPaper(item.dataset.paperUrl);
            }
        });
    }
});
